...
class WebSocket {
	 ...
	 * @return array
	 */
	public function getClientsByPath($path = "/"): array {
	 ...
	 */
	public function connect(string $path = "/", string $origin = "") {
		$this->type = "client";
		$this->create();
...
//			sleep(1);
	 ...
	 */
	public function trigger(string $name, array $arguments) {
	 ...
	 * @param function $function Callback function
	 */
	public function one(string $name, callable $function) {
		$this->events[] = [
			"name" => $name,
			"function" => $function,
			"type" => "one",
	 ...
	 * @param string $data
	 */
	public function sendData($client, string $data) {
	 ...
	 * @return self
	 */
	public function disconnectClient($client, int $statusCode = self::STATUS_CLOSE_NORMAL, string $reason = ""): self {
	 ...
	 * @param object $client
	 */
	private function log(string $message, $client = null): self {
	...
	private function checkOrigin($headers): bool {
	...
	private function sendHttpResponse($client, int $httpStatusCode = 400) {
		$httpHeader = 'HTTP/1.1 ';
	...
	private function processClientHandshake($client, string $input): bool {
		$matches = [];
...
//		socket_clear_error($socket);
	...
	private function hybi10Encode($client, string $payload, string $type = 'text', bool $masked = false): string {
		$frameHead = array();
		$frame = '';
