...
- fixed bug in `HemiFrame\Lib\Cache\Apc`: replaced `apcu_add` with `apcu_store` in `set()` so existing keys are overwritten (`src/HemiFrame/Lib/Cache/Apc.php`)
- updated lockfile dependencies (`composer.lock`)
...
- fixed bug in Template switcher parsing for cases with empty inner HTML (`src/HemiFrame/Template.php`)
